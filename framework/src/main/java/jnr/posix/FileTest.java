...
public class FileTest {
    ...
    public void futimeTest() throws Throwable {
        if (!Platform.IS_WINDOWS) {
            File f = File.createTempFile("jnr-PosixDriver-futime", "tmp");
            long oldTime = posix.stat(f.getAbsolutePath()).mtime();
            Thread.sleep(2000);
    ...
    public void fchmodTest() throws IOException {
        if (!Platform.IS_WINDOWS) {
            File tmp = File.createTempFile("jnr-PosixDriver-chmod-test", "tmp");
    ...
    public void renameTest() throws IOException {
        File oldFile = File.createTempFile("jnr-PosixDriver-rename-test", "tmp");
        File newFile = new File(oldFile.getParent() + File.separatorChar + "jnr-PosixDriver-rename-test-new");
    ...
    public void accessTest() throws IOException {
        File tmp = File.createTempFile("jnr-PosixDriver-access-test", "tmp");
